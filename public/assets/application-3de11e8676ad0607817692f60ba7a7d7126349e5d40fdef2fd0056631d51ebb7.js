...
// Addition of Ailments to Patients General Details Block
    ...
    function loadTemplateForAilment(ailment_name) {
        addTab($("#patient_detail_main"),1,ailment_name,"This is the Tab for Diabeties Ailment");
...
function simpleComboLoad(urlToLoadCombo,comboElementToLoad,nameNameInOption,callbackFunction){
    $.ajax({
        type:"GET",
        url:urlToLoadCombo,
        success:function(comboResultArray){
            ...
            if (typeof callback === "function") {
                callbackFunction.apply(comboResultArray);
            }
        }
    });
}
...
var addTab = function (tabs, tabId, tabLabel, tabContentHtml) {
    ...
    var header = "<li><a id='"+ailment_tab_id+"' href='#" + tabId + "'>" + tabLabel + "&nbsp;&nbsp;<i class='fa fa-remove' style='cursor:pointer;' title='Delete Ailment - "+tabLabel+"' id='"+delete_ailment_id+"' onclick='removeTab("+ailment_tab_id+","+tabs[0].id+","+tabId+");'></i> </a> </li>";
...
function add_new_comorbid_condition(comorbid_condition_selected) {
    console.log(comorbid_condition_selected);
}
;
(function() {
...
function loadAilments(){
    ...
    simpleComboLoad("/get_all_ailments_for_combo.json",$('#ailment_combo_select_id'),true,);
}
...
// Non-Edit Patient Template Loading
    function loadNonEditPatientTemplates(patientDetails){
        ...
        loadNonEditPatientPGDTemplate(patientDetails);
    ...
    function loadNonEditPatientPGDTemplate(patientDetails){
        ...
        // var spans = $('form span');
        // var all_ids = $('#patient_gen_details_main_div [id^="input_pgd_"]');
        ...
        $.each(patientDetails,function (eachpatientDetailKey,eachpatientDetailValue) {
            ...
            var eachpatientDetailInputElement = $('#input_pgd_'+eachpatientDetailKey);
            var eachpatientDetailSpanElement = $('#span_pgd_'+eachpatientDetailKey);
            console.log(eachpatientDetailInputElement);

            if($(eachpatientDetailInputElement).is("input[type=file]")){
                var file_element = $('#span_pgd_'+eachpatientDetailKey+"_src")
                $(file_element).attr("src",eachpatientDetailValue);
            }else if($(eachpatientDetailInputElement).is("input[type=checkbox]")){
                var checkboxValue = "Yes";
                var checkedValue = true;
                console.log(eachpatientDetailKey, eachpatientDetailValue);
                if(!eachpatientDetailValue){
                    checkboxValue = "No";
                    checkedValue = false;
                }
                ...
                $(eachpatientDetailSpanElement).text(eachpatientDetailValue);
                $(eachpatientDetailInputElement).val(eachpatientDetailValue);
